...
export async function sendViaFast2Sms(
...
): Promise<SmsProviderResult & { provider?: string }> {
  const apiKey =
  ...
  try {
    ...
    // If DLT route failed due to unapproved sender ID, retry with Quick SMS route (route='q')
    const initialError = Array.isArray(data.message) ? data.message.join(', ') : data.message || 'DLT error';
    if (initialError.toLowerCase().includes('sender id') || initialError.toLowerCase().includes('dlt') || data.status_code === 406) {
      console.log(`[Fast2SMS] DLT route requires approved header (${initialError}). Retrying via Quick SMS route (route=q)...`);
      try {
        ...
        const qSuccess = qData.return === true || qData.return === 'true' || (typeof qData.return === 'boolean' && qData.return);
        if (qSuccess) {
          const qMsgId = qData.request_id || (Array.isArray(qData.message) ? qData.message[0] : qData.message) || `f2s_q_${Date.now()}`;
          console.log(`[Fast2SMS Quick Route Success] ID: ${qMsgId} | To: ${maskedPhone}`);
          return {
            success: true,
            provider: 'fast2sms',
            messageId: String(qMsgId),
            rawResponse: qData,
          };
        }
      } catch (qErr: any) {
        console.warn('[Fast2SMS Quick Route Error]:', qErr.message);
      }
    }
