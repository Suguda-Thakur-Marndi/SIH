export async function sendSms(phone: string, message: string, notificationId?: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
  ...
  try {
    // Normalize phone number (assume Indian +91 if missing country code)
...
async function sendViaMsg91(phone: string, message: string) {
  ...
  // Simplified MSG91 API call for hackathon. 
  // Note: MSG91 flow API typically expects variable mapping. We assume a generic template that takes one large `message` var or direct text.
  const response = await fetch('https://api.msg91.com/api/v5/flow/', {
    ...
    body: JSON.stringify({
      ...
      recipients: [
        {
          mobiles: phone,
          message: message // Provided the template expects this variable
        }
      ]
    })
  });
